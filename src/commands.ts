...
async function checkTypescript(): Promise<void> {
    log.step(1, 2, 'Starting TypeScript check');
...
async function checkEslint(fix = false): Promise<void> {
    log.step(2, 2, `Starting ESLint ${fix ? 'fix' : 'check'}`);
...
async function commitLint(): Promise<void> {
    log.step(null, null, 'Running commit lint');
...
async function setupGitHook(): Promise<void> {
    log.step(2, 2, 'Setting up Git hooks');
...
async function setupProject(): Promise<void> {
    log.step(1, 2, 'Starting project setup');
