...
| **Security**        | Centralized Rate Limiting | Medium | M          | Investigate `graphqlUploadExpress` rate limits. As configured in `src/node/express/express.util.ts`, the current 1000 requests/15 min limit may cause false-positive DoS mitigation on normal GraphQL query volumes. Recommend separating REST and GraphQL rate limiting policies. |
| **Maintainability** | ESLint Configuration      | Low    | S          | Remove unused ESLint `dist/config/index.js` dependency in `eslint.config.js` (flat config) or ensure the pre-build step is explicitly documented, as it caused local `eslint` commands to fail out of the box when `dist` was not populated.                                   |
...
3.  **Inject Design Docs:** When available, inject the relevant Mongo migration design doc(s) into my context to clarify whether the parallel Mongoose/Native driver setup is an ongoing migration or a permanent architectural decision. This will improve the accuracy of my architectural recommendations.
